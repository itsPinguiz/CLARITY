...
def smart_resampling(
    ...
    **kwargs,
    ...
    Computes a per-class target from *strategy*, then:
    - Applies ``semantic_downsampling`` to all classes above the target.
    - Applies ``paraphrase_upsampling`` to all classes below the target.
    - Classes already at the target size are left untouched.

    This is the recommended entry point when you want a fully balanced
    training set with maximum semantic diversity while avoiding the
    truncation bias of random undersampling and the redundancy of simple
    oversampling.

    Args:
        ds                    : DatasetDict with train / validation / test.
        label2id              : class → integer id mapping.
        strategy              : how to compute the per-class target.
                                "mean" | "median" | "majority" | int.
        embed_column          : text column for semantic embeddings
                                (passed to semantic_downsampling).
        paraphrase_column     : text column whose content is paraphrased
                                (passed to paraphrase_upsampling).
        embed_model_name      : SentenceTransformer model name.
        paraphrase_model_name : HuggingFace Seq2Seq paraphrase model.
        batch_size            : GPU batch size for paraphrase generation.
        seed                  : reproducibility seed.

    Returns:
        DatasetDict with a balanced training split.
    """
    counts = Counter(ds["train"][LABEL_COLUMN])
    target = _compute_target(counts, strategy)

    over_represented = {lbl for lbl, c in counts.items() if c > target}
    under_represented = {lbl for lbl, c in counts.items() if c < target}

    print(
        f"[smart_resampling] Strategy : '{strategy}' → target = {target}\n"
        f"  Over-represented  : {over_represented or '(none)'}\n"
        f"  Under-represented : {under_represented or '(none)'}"
    )
    ...
    if under_represented:
        result_ds = paraphrase_upsampling(
            ...
            model_name=paraphrase_model_name,
